...
# Prepended to the README copied onto the published branch. The README's links are relative
# to the repository root, and nothing they point at exists here, so the copy says where it
# came from rather than offering a page of dead links with no explanation.
README_NOTE = """<!-- Generated by build_static.py from README.md on `main`. Do not edit here. -->

> **This is the `gh-pages` branch** &mdash; the console, pre-rendered. The site it serves is at
> <https://ogheneruno.github.io/ClearQueue/>. The source, and every file the links below point
> at, live on [`main`](https://github.com/Ogheneruno/ClearQueue/tree/main).
...
def build(out: Path) -> None:
    ...
    # The README, so the branch describes itself on GitHub instead of reading as an
    # unexplained pile of JSON. Regenerated on every build, which is what stops the wipe
    # above from quietly losing it.
    (out / "README.md").write_text(
        README_NOTE + (ROOT / "README.md").read_text(encoding="utf-8"), encoding="utf-8")
